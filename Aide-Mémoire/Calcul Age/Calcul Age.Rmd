...
performance_nobs <- 1e3  # Nombre d'observations du tableau

# Taux de succès
tx_succ_date_fin <- "2020-12-31"  # Date de fin pour analyse des taux de succès
...
```
Diviser par 10000 et prendre la valeur plancher fonctionnera toujours pour calculer l'âge. **Notez bien** qu'il est important de prendre la valeur plancher (`floor()`) parce que les décimales n'ont pas de sens analytique.
...
1) Calculer le nombre d'années entre la date de calcul et la date de naissance.  
2) Si la combinaison `mois-jour` de la date de calcul est plus petite que celle de la date de naissance soustraire 1 à la valeur précédemment calculée.

Exemple où l'anniversaire est le 10 mars 2000 et l'année de calcul est en 2020.
1) 2020 - 2000 = 20  
2) Soustraire 1 à l'âge calculé si la date est entre le 1$^{er}$ janvier et le 9 mars.  
Cette méthode considère que l'anniversaire d'une naissance le 29 février sera le 1$^{er}$ mars lors des années non bissextiles. Dans certain pays c'est le 28 février.
...
### NOTE : df pour data.frame (dplyr) et dt pour data.table ###
# Diviser par 365.25
...
# Valeur de la colonne référence - méthode Comparaison Mois-Jour
dt[, REF := fcase(
  month(CALCUL) < month(NAISS) | (month(CALCUL) == month(NAISS) & day(CALCUL) < day(NAISS)),
    year(CALCUL) - year(NAISS) - 1,
  default = year(CALCUL) - year(NAISS)
)]
# Valeur des autres méthodes (data.table uniquement)
...
for (col in (cols)) {
  ...
  tx_succ <- dt[, .(METHODE = col,
                    TX_SUCC = round(sum(get(paste0("Tx", col))) / .N * 100, 2))]
  TxSucc[[col]] <- tx_succ
}
...
## Échantillon âge incorrect

```{r, echo=FALSE}
cols <- c("_36525", "_365", "_int_dyr", "_dec_date")
annexe_ech_inc <- vector("list", length(cols))
for (i in 1:length(cols)) {
  annexe_ech_inc[[i]] <- dt[REF != get(cols[i])][1:10]
}
annexe_ech_inc <- unique(rbindlist(annexe_ech_inc))[, .(NAISS, CALCUL, REF, `_36525`, `_365`, `_int_dyr`, `_dec_date`)]
kbl(annexe_ech_inc) %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive")) %>%
  column_spec(4, background = ifelse(annexe_ech_inc$`_36525` != annexe_ech_inc$REF, "red", "white")) %>%
  column_spec(5, background = ifelse(annexe_ech_inc$`_365` != annexe_ech_inc$REF, "red", "white")) %>%
  column_spec(6, background = ifelse(annexe_ech_inc$`_int_dyr` != annexe_ech_inc$REF, "red", "white")) %>%
  column_spec(7, background = ifelse(annexe_ech_inc$`_dec_date` != annexe_ech_inc$REF, "red", "white"))
