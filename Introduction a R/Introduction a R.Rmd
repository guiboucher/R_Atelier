...
- Les valeurs décimales comme `4.5` sont numériques (`numeric`/`num`).
- Il y a les nombres entiers comme 4 (`integer`/`int`).
- Les valeurs booléennes `TRUE` ou `FALSE` sont appelées logiques (`logical`/`logi`)
- Les valeurs textes sont appelées caractères (`character`/`chr`)

```{r}
var_numeric <- 4
var_integer <- 4L
var_logic <- TRUE
var_character <- "Chaine de caractères"
```
