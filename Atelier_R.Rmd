...
Est-ce qu'on met en commentaire vingt lignes de codes `setwd(...)` au début de chaque script?  
...
Vous a-t-on déjà dit que les boucles en R ne sont pas rapides? Qu'il est préférable d'utiliser un `lapply` ou un `sapply`?
...
n <- 1e3
microbenchmark(fct_sapply(n), fct_loop(n))
```
...
```{r}
fct_loop <- function(n) {
  vec <- c() # Créer un vecteur : 1 étape
  for (i in 1:n) {
    vec <- c(  # (1) Recréer vecteur
      vec,  # (2) Placer les anciennes valeurs dans vecteur
      2*i+7  # (3) Placer la nouvelle valeur
    )  # -> 3 étapes x n = 3n étapes
  }
  vec
}
```
Total de `3n+1` étapes
...
fct_loop2 <- function(n) {
  vec <- vector("integer", n)  # 1 étape
  for (i in 1:n) {
    vec[i] <- 2*i+7  # (1) Placer la nouvelle valeur dans 'vec'
  }  # -> 1 étape x n = n étapes
}
```
Total de 
